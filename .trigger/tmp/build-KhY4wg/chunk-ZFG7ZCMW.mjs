  ...
  "node_modules/@supabase/functions-js/dist/main/helper.js"(exports) {
    ...
    var resolveFetch = /* @__PURE__ */ __name((customFetch) => {
      ...
      if (customFetch) {
        ...
        _fetch = /* @__PURE__ */ __name((...args) => Promise.resolve(`${"@supabase/node-fetch"}`).then((s) => __importStar(__require(s))).then(({ default: fetch2 }) => fetch2(...args)), "_fetch");
      } else {
        _fetch = fetch;
      }
      ...
      constructor(endPoint, options) {
        ...
        this._resolveFetch = (customFetch) => {
          ...
          if (customFetch) {
            ...
            _fetch = /* @__PURE__ */ __name((...args) => Promise.resolve(`${"@supabase/node-fetch"}`).then((s) => __importStar(__require(s))).then(({ default: fetch2 }) => fetch2(...args)).catch((error) => {
  ...
  "node_modules/@supabase/storage-js/dist/main/lib/helpers.js"(exports) {
    ...
    var resolveResponse = /* @__PURE__ */ __name(() => __awaiter(void 0, void 0, void 0, function* () {
      if (typeof Response === "undefined") {
        return (yield Promise.resolve(`${"@supabase/node-fetch"}`).then((s) => __importStar(__require(s)))).Response;
      }
      return Response;
    }), "resolveResponse");
  ...
  "node_modules/@supabase/supabase-js/dist/main/lib/helpers.js"(exports) {
    ...
    function applySettingDefaults(options, defaults) {
      ...
      const result = {
        ...
        accessToken: /* @__PURE__ */ __name(() => __awaiter(this, void 0, void 0, function* () {
          return "";
        }), "accessToken")
      };
...
var require_main5 = __commonJS({
  "node_modules/@supabase/supabase-js/dist/main/index.js"(exports) {
    ...
    var createClient2 = /* @__PURE__ */ __name((supabaseUrl2, supabaseKey, options) => {
...
// src/features/pricing-engine/services/pricingAlgorithm.ts
async function runPricingAlgorithm() {
  const stats = { processed: 0, increased: 0, reverted: 0, waiting: 0 };
  const errors = [];
  try {
    const { data: globalSetting } = await supabaseAdmin.from("global_settings").select("value").eq("key", "smart_pricing_global_enabled").single();
    ...
    const { data: products } = await supabaseAdmin.from("products").select(`*, pricing_config!inner(*)`).eq("pricing_config.auto_pricing_enabled", true);
...
__name(runPricingAlgorithm, "runPricingAlgorithm");
async function processProduct(product, config, stats) {
...
__name(processProduct, "processProduct");
async function getRevenue(productId, periodHours) {
  const now = /* @__PURE__ */ new Date();
  const currentStart = new Date(now.getTime() - periodHours * 60 * 60 * 1e3);
  const previousStart = new Date(currentStart.getTime() - periodHours * 60 * 60 * 1e3);
  const { data: currentData } = await supabaseAdmin.from("sales_data").select("revenue").eq("product_id", productId).gte("date", currentStart.toISOString().split("T")[0]);
  const { data: previousData } = await supabaseAdmin.from("sales_data").select("revenue").eq("product_id", productId).gte("date", previousStart.toISOString().split("T")[0]).lt("date", currentStart.toISOString().split("T")[0]);
...
__name(getRevenue, "getRevenue");
async function increasePrice(product, config, stats, revenue) {
  ...
  if (percentIncrease > config.max_increase_percentage) {
    ...
    await supabaseAdmin.from("pricing_config").update({ current_state: "at_max_cap" }).eq("product_id", product.id);
    stats.increased++;
    return;
  }
...
__name(increasePrice, "increasePrice");
async function revertPrice(product, config, stats, revenue) {
  const { data: history } = await supabaseAdmin.from("pricing_history").select("old_price").eq("product_id", product.id).eq("action", "increase").order("timestamp", { ascending: false }).limit(1).single();
  ...
  await supabaseAdmin.from("pricing_config").update({
    ...
    next_price_change_date: waitUntil.toISOString()
    // Wait until this date
  }).eq("product_id", product.id);
  stats.reverted++;
}
__name(revertPrice, "revertPrice");
async function updatePrice(product, config, newPrice, action, reason, revenue) {
  await updateShopifyPrice(product.shopify_id, newPrice);
  await supabaseAdmin.from("products").update({ current_price: newPrice }).eq("id", product.id);
  const now = /* @__PURE__ */ new Date();
  const nextChange = new Date(now.getTime() + config.period_hours * 60 * 60 * 1e3);
  await supabaseAdmin.from("pricing_config").update({
...
async function updateShopifyPrice(shopifyId, newPrice) {
  ...
  const updateRes = await fetch(`${baseUrl}/variants/${variantId}.json`, {
    ...
    body: JSON.stringify({
      variant: {
        ...
        // Clear "compare at" price to avoid crossed-out effect
      }
    })
  });
  if (!updateRes.ok) throw new Error(`Failed to update price: ${updateRes.statusText}`);
}
__name(updateShopifyPrice, "updateShopifyPrice");

// src/trigger/daily-pricing.ts
var dailyPricingTask = schedules_exports.task({
